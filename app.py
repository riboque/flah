...
VERSÃO COM BANCO DE DADOS ONLINE - Suporte a PostgreSQL, MySQL, SQLite
"""
...
print(f"[DB] → Usando: {app.config.get('SQLALCHEMY_DATABASE_URI', 'N/A')[:50]}...")
...
from routes.monitor_routes import monitor_bp, init_shared_data as init_monitor_data, set_db_manager as set_monitor_db
from routes.main_routes import main_bp, init_shared_data as init_main_data, set_db_manager
...
# Blueprint do banco de dados
from routes.db_routes import db_routes_bp, init_db_routes
init_db_routes(db_manager)
...
set_db_manager(db_manager)  # Passar db_manager para verificação de admin
set_monitor_db(db_manager)  # Passar db_manager para o monitor
...
# Registrar blueprint do banco de dados
...
def handle_general_error(error):
    ...
    # Em modo debug, propagar a exceção
    if app.debug:
        raise error
